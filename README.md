...
- 可將目前排列和旋轉儲存為最新版 PDF
- 「常用 PDF 工具」分頁已補回批次工具：合併、拆分 ZIP、進階拆分（每 N 頁 / 依範圍各存一檔）、抽頁、刪頁、旋轉、加密、加密+權限控制（禁止列印 / 複製 / 修改）、Bates 編號、解密、壓縮、抽文字、OCR 抽文字、掃描 PDF 轉可搜尋 PDF、圖片轉 PDF、PDF 轉圖片、PDF 資訊、頁碼、水印、刪空白頁、清理 Metadata
