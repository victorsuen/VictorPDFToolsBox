...
- 「文字編輯 Beta」分頁可偵測 PDF 文字層、跨頁上一個 / 下一個搜尋文字、區分大小寫 / 全字匹配、在預覽高亮匹配項、合併同一行碎片、在預覽中點擊選取文字片段，並即時預覽替換覆蓋效果；支援覆蓋替換、選取文字遮蔽、批量遮蔽搜尋結果與實驗性直接內容流替換
...
PDF 的文字層，將同一行的碎片合併成較完整的文字片段，讓你用上一頁 / 下一頁導覽、跨頁上一個 / 下一個搜尋、區分大小寫 / 全字匹配、目前頁與整份文件符合數、預覽高亮與清除、清單或預覽點擊選取；輸入替換文字時會用接近實際輸出的覆蓋框即時預覽效果，並顯示直接內容流替換的長度/字元相容提示。預設的「覆蓋替換」
會用白底覆蓋原字並放入新文字，是較穩定做法；「遮蔽選取文字」預設會用黑框蓋住選取區塊，也可依搜尋字串批量遮蔽所有匹配片段，亦可選用
實驗性的安全遮蔽，對簡單英文 / 數字 / 半形符號先移除底層文字再加黑框。「直接改內容流」仍屬實驗功能，
目前只支援簡單英文 / 數字 / 半形符號，且新舊文字長度需相同，好處是可保留原 PDF 文字樣式。掃描 PDF
