...
- 「文字編輯 Beta」分頁可偵測 PDF 文字層、搜尋文字、合併同一行碎片、在預覽中點擊選取文字片段，支援覆蓋替換與實驗性直接內容流替換
...
會用白底覆蓋原字並放入新文字，是較穩定做法；「直接改內容流」仍屬實驗功能，目前只支援簡單英文 /
數字 / 半形符號，且新舊文字長度需相同，好處是可保留原 PDF 文字樣式。掃描 PDF 請先使用 OCR 轉成
可搜尋 PDF，再嘗試文字編輯。
