...
- 可將目前排列和旋轉儲存為最新版 PDF
- 「常用 PDF 工具」分頁已補回批次工具：合併、拆分 ZIP、進階拆分（每 N 頁 / 依範圍各存一檔）、抽頁、刪頁、旋轉、加密、加密+權限控制（禁止列印 / 複製 / 修改）、Bates 編號、解密、壓縮、**進階壓縮**、抽文字、OCR 抽文字、掃描 PDF 轉可搜尋 PDF、圖片轉 PDF、PDF 轉圖片、PDF 資訊、頁碼、**浮水印（文字／位置／角度／透明度）**、刪空白頁、清理 Metadata、**插入頁面**、**取代頁面**、**比對 PDF 文字**、**視覺比對 PDF**、**依書籤拆分**、**影像印章 / 簽名**、**壓平表單**、**搜尋並螢光**、**安全塗銷關鍵字**
...
- 插入 / 取代頁面、進階壓縮、PDF 文字比對、視覺比對、依書籤拆分、影像印章 / 簽名、填表、壓平表單、搜尋螢光標註、安全塗銷關鍵字
...
- **工具**：拖曳框選位置後套用簽名影像、文字圖章或圖章庫圖片、進階壓縮、比對 PDF 文字／畫面、依書籤拆分
