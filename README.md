...
- 直接把 PDF 拖入縮圖區
- **「文件工作台」分頁（第一個主 Tab）**：類 Acrobat 的閱讀 / 註解 / 編輯 / 整理 / 裁切 / 工具模式；左側縮圖與書籤（可編輯目錄）、中央頁面預覽（縮放、復原）、右側依模式切換工具
...
- 可將目前排列和旋轉儲存為最新版 PDF
- 「常用 PDF 工具」分頁已補回批次工具：合併、拆分 ZIP、進階拆分（每 N 頁 / 依範圍各存一檔）、抽頁、刪頁、旋轉、加密、加密+權限控制（禁止列印 / 複製 / 修改）、Bates 編號、解密、壓縮、**進階壓縮**、抽文字、OCR 抽文字、掃描 PDF 轉可搜尋 PDF、圖片轉 PDF、PDF 轉圖片、PDF 資訊、頁碼、水印、刪空白頁、清理 Metadata、**插入頁面**、**取代頁面**、**比對 PDF 文字**、**依書籤拆分**、**影像印章 / 簽名**、**壓平表單**、**搜尋並螢光**、**安全塗銷關鍵字**
...
- **延遲載入縮圖**：開啟大 PDF 時先顯示「第 N 頁」文字項目，再分批非同步補上縮圖（優先目前頁附近）
- **文字圖章**：工作台「工具」模式與常用工具 `文字圖章`；預設 APPROVED / CONFIDENTIAL / DRAFT / COPY 或自訂，可框選位置後蓋章
- **進階分頁**：文字標註、螢光註解、裁切、文字編輯 Beta、書籤等五個分頁收合到主 Tab「**進階分頁**」
...
- 插入 / 取代頁面、進階壓縮、PDF 文字比對、依書籤拆分、影像印章 / 簽名、壓平表單、搜尋螢光標註、安全塗銷關鍵字

## 文件工作台

啟動桌面版後，第一個主 Tab「**文件工作台**」提供接近 Acrobat 的單檔工作區：

- **閱讀**：搜尋關鍵字並螢光標註，或安全塗銷關鍵字
- **註解**：在預覽上拖曳框選加入螢光 / 底線 / 圖形 / 便利貼，累積後一次套用；支援復原
- **編輯**：列出目前頁文字區塊、點擊預覽選取，並無痕替換；可壓平表單欄位
- **整理**：插入 / 取代頁面、旋轉或刪除目前頁
- **裁切**：框選或輸入 L/B/R/T 邊界，套用至目前頁 / 全部 / 自訂頁碼
- **工具**：拖曳框選位置後套用簽名影像或文字圖章（APPROVED / CONFIDENTIAL / DRAFT / COPY）、進階壓縮、比對 PDF 文字、依書籤拆分

預覽列提供 **− / + / 適合頁面** 縮放（50%–300%），並支援 **`Ctrl`+滾輪** 縮放；**`←`/`→`/`PageUp`/`PageDown`** 換頁；頂部 **復原** 或 **`Ctrl+Z`** 可還原標註或重新載入先前的 PDF 路徑。左側 **書籤** 分頁可新增 / 更新 / 刪除 / 排序書籤並另存目錄。
