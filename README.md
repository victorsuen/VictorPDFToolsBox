...
- 「文字編輯 Beta」分頁：在預覽上直接點文字編輯；改完點空白處或按 Enter 即套用，Ctrl+Z 可復原；編碼亂碼改從畫面辨識
...
- **點空白即套用**：文字編輯改完後點頁面空白處或按 Enter，會立刻寫入預覽中的 PDF，不必每次另存
- **Ctrl+Z 復原**：可還原上一筆文字修改；輸入框內則先還原剛打的字
- 套用時略過整頁 OCR 與字型壓縮，速度較快；完成後再按「更新並儲存 PDF」另存
