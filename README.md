...
- 直接把 PDF 拖入縮圖區
- **「文件工作台」分頁（第一個主 Tab）**：類 Acrobat 的閱讀 / 註解 / 編輯 / 整理 / 工具模式；左側縮圖與書籤、中央頁面預覽、右側依模式切換工具（搜尋螢光、安全塗銷、手繪註解、文字區塊替換、插入 / 取代頁面、簽名、進階壓縮、文字比對、依書籤拆分等）
...
- 文件工作台（閱讀 / 註解 / 編輯 / 整理 / 工具；縮圖 + 書籤 + 頁面預覽）
...
- **註解**：在預覽上拖曳框選加入螢光 / 底線 / 圖形 / 便利貼，累積後一次套用
- **編輯**：列出目前頁文字區塊並無痕替換；可壓平表單欄位
- **整理**：插入 / 取代頁面、旋轉或刪除目前頁
- **工具**：Fill & Sign 影像簽名、進階壓縮、比對 PDF 文字、依書籤拆分

左側可切換縮圖清單與書籤樹；儲存後可選擇重新載入輸出檔。Bates 編號等批次工具仍請使用「常用 PDF 工具」分頁。
