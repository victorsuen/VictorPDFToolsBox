...
- 「文字編輯 Beta」分頁可偵測 PDF 文字層、跨頁上一個 / 下一個搜尋文字、在預覽高亮匹配項、合併同一行碎片、在預覽中點擊選取文字片段，支援覆蓋替換、選取文字遮蔽、批量遮蔽搜尋結果與實驗性直接內容流替換
...
PDF 的文字層，將同一行的碎片合併成較完整的文字片段，讓你用跨頁上一個 / 下一個搜尋、預覽高亮、清單或預覽點擊選取。預設的「覆蓋替換」
